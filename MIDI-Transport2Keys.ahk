...
; MIDI HANDLING PART FUNCTIONS
; FOR RUNTIME CODE, LOOK AT THE BOTTOM
...
__MidiInCallback( wParam, lParam, msg )
{
  ...
  if ( midiEvent.status == "NoteOn" && data2 == 127 )
  {
...
start_with_windows:     ; tray menu label
	if fileExist(a_startup "\MIDI-Transport2Keys.lnk")
		fileDelete, % a_startup "\MIDI-Transport2Keys.lnk"
	else fileCreateShortcut, % a_scriptFullPath, % a_startup "\MIDI-Transport2Keys.lnk"
	menu, tray, % fileExist(a_startup "\MIDI-Transport2Keys.lnk") ? ("check") : ("unCheck"), Start with Windows
	return
